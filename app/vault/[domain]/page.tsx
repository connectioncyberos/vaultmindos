  ...
  return (
    <>
      ...
      <Container className="flex flex-col gap-6 py-6">
        ...
        {/* Titulo Principal */}
        <h1 className="text-3xl font-bold leading-tight text-neutral-900">{categoria.name}</h1>

        {/* Descricao */}
        {categoria.description && (
          <p className="text-base leading-relaxed text-neutral-600">{categoria.description}</p>
        )}
        ...
        <section className="mt-3 border-t border-neutral-200 pt-6">
          ...
          {clusters.length === 0 ? (
            <p className="mt-2 text-sm text-neutral-600">Nenhum cluster com artigos ainda neste domínio.</p>
          ) : (
        ...
        <section className="mt-3 border-t border-neutral-200 pt-6">
          ...
          {artigos.length === 0 ? (
            <p className="mt-2 text-sm text-neutral-600">Nenhum artigo publicado ainda neste domínio.</p>
          ) : (
            <div className="mt-4 grid grid-cols-1 gap-4 sm:grid-cols-2">
              {artigos.map((artigo) => (
